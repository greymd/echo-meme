...
docker run -i greymd/tar2rpm < "${THIS_DIR}"/echo-meme.tar.gz > "${THIS_DIR}/echo-meme-${EM_VERSION}.rpm"
docker run -i greymd/tar2deb < "${THIS_DIR}"/echo-meme.tar.gz > "${THIS_DIR}/echo-meme-${EM_VERSION}.deb"
